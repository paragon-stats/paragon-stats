...
vendored copy pinned at v2.2.1 (CI fails on drift), read by the commit/PR-title validator;
...
- Branch names follow `<type>/<issue#>-<short-kebab-summary>` (e.g. `ci/14-protected-trunk`) —
  same `type` set as commits. Enforced by the `branch-name` check; bot-branch exemptions
  are the shared action's default (`dependabot/**` and the like). Multi-issue branches use
  the lead issue.
- Merge commits only (squash and rebase are disabled): a merge preserves every branch
  commit's message and signature, and the release tooling parses branch commits, not PR
  titles.
